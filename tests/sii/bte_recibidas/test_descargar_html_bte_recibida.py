...
class TestDescargarHtmlBteRecibida(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_descargar_html_bte_recibida(self):
        try:
            ...
            html = self.client.html(boletas[0]['codigo'])

            self.assertIsNotNone(html)
            # La API responde el HTML crudo en ISO-8859-1, no JSON.
            self.assertIsInstance(html, bytes)

            if self.verbose:
                print('test_html(): bytes', len(html))
