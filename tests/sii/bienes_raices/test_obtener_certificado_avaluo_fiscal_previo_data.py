...
class TestObtenerCertificadoAvaluoPrevioData(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_obtener_certificado_avaluo_fiscal_previo_data(self):
        try:
            certificado = self.client.certificado_avaluo_fiscal_previo_data(
                self.comuna,
                self.manzana,
                self.predio,
                self.eac,
            ...
            self.assertIn('datos_generales', certificado)

            if self.verbose:
                print(
                    'test_avaluo_fiscal_previo_data(): certificado',
                    certificado,
                )
