...
class TestDescargarCertificadoAvaluoPrevioPdf(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_descargar_certificado_avaluo_fiscal_previo_pdf(self):
        try:
            pdf = self.client.certificado_avaluo_fiscal_previo_pdf(
                self.comuna,
                self.manzana,
                self.predio,
                self.eac,
            ...
            if self.verbose:
                print('test_avaluo_fiscal_previo_pdf(): bytes', len(pdf))
