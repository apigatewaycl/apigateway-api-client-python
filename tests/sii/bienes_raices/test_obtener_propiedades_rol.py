...
class TestObtenerPropiedadesRol(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_obtener_propiedades_rol(self):
        try:
            respuesta = self.client.propiedades_rol(
                self.comuna,
                self.manzana,
                self.predio,
            )
            ...
            self.assertEqual(propiedades[0]['comunaCnp'], self.comuna)

            if self.verbose:
                print('test_propiedades_rol(): propiedades', propiedades)
