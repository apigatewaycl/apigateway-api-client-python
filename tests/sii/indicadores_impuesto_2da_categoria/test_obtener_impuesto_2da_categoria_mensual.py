...
class TestObtenerImpuesto2daCategoriaMensual(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.verbose = bool(int(getenv('TEST_VERBOSE', '0')))
        cls.client = ImpuestoSegundaCategoria()
        cls.periodo = getenv('TEST_IMPUESTO_2DA_PERIODO', '202410').strip()

    def test_obtener_impuesto_2da_categoria_mensual(self):
        try:
            respuesta = self.client.mensual(self.periodo)
            ...
            if self.verbose:
                print('test_impuesto_2da_mensual(): mensual', mensual)
