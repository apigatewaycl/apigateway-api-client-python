...
class TestAnularFoliosCaf(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        ...
        folio_inicial = getenv('TEST_CAF_ANULAR_INICIAL', '').strip()
        folio_final = getenv('TEST_CAF_ANULAR_FINAL', '').strip()
        if not (folio_inicial and folio_final):
            raise unittest.SkipTest(
                'Se requieren TEST_CAF_ANULAR_INICIAL y '
                'TEST_CAF_ANULAR_FINAL: anular folios es irreversible '
                'en el SII.'
            )
    ...
    def test_anular_folios_caf(self):
        try:
            anulacion = self.client.anular(
                self.emisor,
                self.dte,
                self.folio_inicial,
                self.folio_final,
            ...
            self.assertIsNotNone(anulacion)

            if self.verbose:
                print('test_anular(): anulacion', anulacion)
