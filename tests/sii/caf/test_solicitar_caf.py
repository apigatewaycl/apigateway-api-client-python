...
class TestSolicitarCaf(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        ...
        cantidad = getenv('TEST_CAF_CANTIDAD', '').strip()
        if not cantidad:
            raise unittest.SkipTest(
                'TEST_CAF_CANTIDAD no configurado: solicitar folios '
                'consume timbraje real en el SII.'
            )
        cls.cantidad = int(cantidad)
        cls.client = Caf(firma, firma_clave)

    def test_solicitar_caf(self):
        try:
            xml = self.client.solicitar(
                self.emisor,
                self.dte,
                self.cantidad,
            )
