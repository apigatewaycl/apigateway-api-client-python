...
class TestListarSolicitudesCaf(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    # `pagina` parte en 1 y la respuesta trae en
    # `metadata.siguiente_pagina` la siguiente, o None si es la última.
    def test_listar_solicitudes_caf(self):
        try:
            respuesta = self.client.solicitudes(self.emisor, self.dte, 1)
            solicitudes = respuesta['data']

            self.assertIsNotNone(solicitudes)
            self.assertIn('metadata', solicitudes)

            if self.verbose:
                print('test_solicitudes(): solicitudes', solicitudes)
