...
class TestObtenerEstadoFolioCaf(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        ...
        folio = getenv('TEST_CAF_FOLIO', '').strip()
        if not folio:
            raise unittest.SkipTest(
                'TEST_CAF_FOLIO no configurado: se necesita un folio '
                'ya timbrado para consultar su estado.'
            )
        cls.folio = int(folio)
        cls.client = Caf(firma, firma_clave)

    def test_obtener_estado_folio_caf(self):
        try:
            respuesta = self.client.estado(self.emisor, self.dte, self.folio)
