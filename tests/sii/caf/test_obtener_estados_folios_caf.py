...
class TestObtenerEstadosFoliosCaf(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        ...
        if not (folio_inicial and folio_final):
            raise unittest.SkipTest(
                'Se requieren TEST_CAF_FOLIO_INICIAL y '
                'TEST_CAF_FOLIO_FINAL de un CAF ya solicitado.'
            )
        cls.folio_inicial = int(folio_inicial)
        cls.folio_final = int(folio_final)
        cls.estado = getenv('TEST_CAF_ESTADO', 'recibidos').strip()
        cls.client = Caf(firma, firma_clave)

    def test_obtener_estados_folios_caf(self):
        try:
            estados = self.client.estados(
                self.emisor,
                self.dte,
                self.folio_inicial,
                self.folio_final,
            ...
            self.assertIsNotNone(estados)

            if self.verbose:
                print('test_estados(): estados', estados)
