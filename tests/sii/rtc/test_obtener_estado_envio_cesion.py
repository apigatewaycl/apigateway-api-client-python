...
class TestObtenerEstadoEnvioCesion(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        ...
        cls.track_id = getenv('TEST_RTC_TRACK_ID', '').strip()
        if not cls.track_id:
            raise unittest.SkipTest(
                'TEST_RTC_TRACK_ID no configurado: se necesita el track '
                'id del envío de una cesión real.'
            )

    def test_obtener_estado_envio_cesion(self):
        try:
            estado = self.client.estado_envio(self.track_id)['data']
