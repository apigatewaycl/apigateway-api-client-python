...
class TestObtenerEstadosF29(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_obtener_estados_f29(self):
        try:
            estados = self.client.obtener_estados()['data']

            self.assertIsNotNone(estados)
            if estados:
                self.assertIn('periodo', estados[0])
                self.assertIn('declarado', estados[0])

            if self.verbose:
                print('test_obtener_estados(): estados', estados)
