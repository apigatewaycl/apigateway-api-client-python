...
class TestObtenerDetallesDeclaracionF29(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_obtener_detalles_declaracion_f29(self):
        try:
            folio = self._primer_folio()
            detalles = self.client.detalles_declaracion(folio)['data']

            self.assertIsNotNone(detalles)

            if self.verbose:
                print('test_detalles_declaracion(): detalles', detalles)
