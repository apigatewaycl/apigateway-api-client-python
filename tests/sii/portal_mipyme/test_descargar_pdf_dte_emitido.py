...
class TestDescargarPdfDteEmitido(unittest.TestCase):
    ...
    def test_descargar_pdf_dte_emitido(self):
        try:
            ...
            pdf = self.client.pdf(
                self.contribuyente_rut, documentos[0]['codigo']
            )
            ...
            output_dir = os.path.join(base_dir, 'archivos', 'sii', 'mipyme_dte_emitido_pdf')
            ...
            filename = os.path.join(
                output_dir,
                'MIPYME_DTE_EMITIDO_%(contribuyente_rut)s_T%(dte)sF%(folio)s.pdf' % {
