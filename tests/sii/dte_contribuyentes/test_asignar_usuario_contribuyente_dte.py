...
class TestAsignarUsuarioContribuyenteDte(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        ...
        cls.usuario_run = getenv('TEST_DTE_USUARIO_RUN', '').strip()
        if not cls.usuario_run:
            raise unittest.SkipTest(
                'TEST_DTE_USUARIO_RUN no configurado: este test le '
                'asigna permisos reales a un usuario en el SII.'
            )
        cls.client = Contribuyentes(firma, firma_clave)

    def test_asignar_usuario_contribuyente_dte(self):
        try:
            usuario = self.client.set_usuario(
                self.contribuyente_rut,
                {
                    'run': self.usuario_run,
                    'permisos': {
                        'administrador': False,
            ...
            if self.verbose:
                print('test_set_usuario(): usuario', usuario)
