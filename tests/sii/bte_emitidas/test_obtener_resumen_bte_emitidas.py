...
class TestObtenerResumenBteEmitidas(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        ...
        cls.anio = getenv('TEST_BTE_ANIO', cls.periodo[:4]).strip()

    def test_obtener_resumen_bte_emitidas(self):
        try:
            respuesta = self.client.resumen(self.contribuyente_rut, self.anio)
            resumen = respuesta['data']

            self.assertIsNotNone(resumen)

            if self.verbose:
                print('test_resumen(): resumen', resumen)
