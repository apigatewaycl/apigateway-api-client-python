...
class TestListarPeriodosIndicadores(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.verbose = bool(int(getenv('TEST_VERBOSE', '0')))
        cls.client = Indicadores()

    def test_listar_periodos_indicadores(self):
        try:
            respuesta = self.client.periodos()
            self.assertIn('data', respuesta)
            self.assertIn('metadata', respuesta)
            periodos = respuesta['data']

            self.assertIsNotNone(periodos)
            self.assertGreater(len(periodos), 0)
            self.assertIn('url', periodos[0])

            if self.verbose:
                print('test_periodos(): periodos', periodos)
